 ...
 *
 * \subsection enginexoverview Overview
 *
 * This object exposes the RenderWare engine. The engine manages:
 *
 * \li memory handling
 ...
 * \li initializing and terminating RenderWare itself.
 *
 * All RenderWare applications must Initialize, Open, then Start the
 * RenderWare Engine before entering their main loop.
 *
 * All plugins must be registered - by calling their
 * respective "PluginAttach()"  functions - prior to
 * calling \ref RwEngineStart.
 *
 */
...
#ifndef RWGLOBALSIZE
    RwEngineInstance = RwMalloc(engineTKList.sizeOfStruct, rwMEMHINTDUR_GLOBAL);
 ...
 * mechanism for adding plugins to RenderWare. The global data is
 * initialized using the specified object constructor when \ref RwEngineStart
 * is called. Note that a NULL return value from the constructor will cause
 * \ref RwEngineStart to fail.
 *
 * \param size  An \ref RwInt32 value equal to the size of the memory block to
 *              reserve.
 * \param pluginID  A \ref Rwint32 value equal to the plugin ID (must be unique - used to
 ...
 *
 * \return Returns an \ref RwInt32 value equal to the byte offset within the engine
...
/**
 * \ingroup rwengine
 * \ref RwEngineGetNumSubSystems is used to determine the number of
 * rendering subsystems (graphics devices) currently available on the
 * platform the RenderWare engine is running. This function should
...
/**
 * \ingroup rwengine
 * \ref RwEngineGetSubSystemInfo is used to retrieve information about
...
/**
 * \ingroup rwengine
 * \ref RwEngineGetCurrentSubSystem is used to retrieve the index of the
 * currently selected rendering subsystem (graphics device). This function
 * should only be called after the engine has been opened.
 *
...
RwEngineGetCurrentSubSystem(void)
{
    ...
    if (_rwDeviceSystemRequest(&RWSRCGLOBAL(dOpenDevice),
                               ...
                               &subSystemNumber, NULL, numRequest))
    {
 ...
 * The rendering subsystem can only be set after the Graphics
...
/**
 * \ingroup rwengine
 * \ref RwEngineGetNumVideoModes is used to determine the number of
...
/**
 * \ingroup rwengine
 * \ref RwEngineGetVideoModeInfo is used to retrieve information about
 ...
 * only be called after the engine has been opened.
 *
 * \param modeinfo   A pointer to an \ref RwVideoMode value that will receive
 *                  the information.
...
/**
 * \ingroup rwengine
 * \ref RwEngineGetCurrentVideoMode is used to retrieve the index of the
...
/**
 * \ingroup rwengine
 * \ref RwEngineSetVideoMode is used to set the current video mode
 * to the video mode with the specified index. The video mode can only be set
 * after the RenderWare engine has been opened.
 *
...
/**
 * \ingroup rwengine
 * \ref RwEngineGetTextureMemorySize is used to determine the amount of
 * available texture memory.
 *
 * \return Returns an \ref RwInt32 value giving the number of bytes available to
 *        allocate to textures.  Returns -1 if the query failed.
 *
...
/**
 * \ingroup rwengine
 * \ref RwEngineGetMaxTextureSize is used to determine the maximum
 * dimensions of textures allocatable with this device.
 *
 * \return Returns an \ref RwInt32 value giving the maximum dimension (in texels)
 * of textures that may be allocated.  Returns -1 if the query failed.
 *
...
/**
 * \ingroup rwengine
 * \ref RwEngineSetFocus is used to change the focus properties of the
...
/**
 * \ingroup rwengine
 * \ref RwEngineGetMetrics is used to get metrics from RenderWare
 ...
 * engine is as follows: \ref RwEngineInit, \ref RwEngineOpen,
...
RwEngineStart(void)
{
                               ...
                               numRequest))
    {
        ...
        if (_rwPluginRegistryInitObject(&engineTKList, RwEngineInstance))
        {
            ...
            /* Get the device to finalise it's objects
             * (device specific overload time) */
            _rwDeviceSystemRequest(device,
...
/**
 * \ingroup rwengine
 * \ref RwEngineClose is used to close the RenderWare engine by
 ...
 * RenderWare engine is as follows: \ref RwEngineInit,
 ...
 * \ref RwEngineTerm to run-down.
 *
 * \param initParams  A pointer to an \ref RwEngineOpenParams value describing
 *       a device-dependent parameter (see example below)
 *
 * \return Returns TRUE if successful or FALSE if there is an error
 *
 * \see RwEngineInit
...
/**
 * \ingroup rwengine
 * \ref RwEngineInit is used to initialize the core of the RenderWare
 * rendering engine.
 *
 ...
 * RenderWare will use for memory management. If specified,
 * RenderWare will use the supplied memory allocation routines rather
 * than the default standard library ones. Specifying NULL for this parameter
 * forces the default ones to be used.
 *
 * \if gcn
 ...
 * \ref RwEngineClose, \ref RwEngineTerm to run-down.
 *
 * Note that the purpose of FreeLists are to improve memory management
 ...
 * possible.
 *
 * Note too that ultimately RenderWare FreeLists make use of the
 * memory functions supplied as the first parameter. The majority of customers
 * will want to use FreeLists since these increase performance.
 *
 * Running RenderWare without Freelists is allowed purely to help developers
 * observe memory usage since calls to RwMalloc are easily hooked.
 *
 ...
 * RenderWare. Currently the only flag is whether to use Freelists or
 * not. A value of rwENGINEINITNOFREELISTS can be used to tell RenderWare
 * to use \ref RwMalloc and \ref RwFree in place of
 * \ref RwFreeListAlloc and \ref RwFreeListFree.
 *
 * \param resArenaSize    An \ref RwUInt32 specifying the size of the memory block (in bytes)
 * reserved for the resources instancing arena. Can be set to zero if no instance data
 * will be used
 *
 * \return Returns TRUE if successful or FALSE if there is an error
 *
