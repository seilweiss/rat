 ...
 * \ref RTFS_MAX_NAME_LENGTH is the maximum length of a file system name,
 *      including the trailing 0 byte. The file system name passed to any
 *      OS-specific file system must not exceed RTFS_MAX_NAME_LENGTH (please
 *      refer to the OS-specific documentation for more information).
 */
...
enum RtFileAccessFlag
{
   ...
   RTFILE_ACCESS_OPEN_ASYNC  = 0x10,    /**< Flag specifying that a file will
                                             be opened asynchronously. */
   RTFILE_ACCESS_NO_BUFFERING= 0x20,    /**< Flag specifying that the file will
                                             not be automatically buffered
                                             (PS2 only).*/
...
struct RtFile
{
    ...
    RtFSFileOpCallBack              outstandingCB;    /**< Callback triggered
                                                         ...
                                                         operation completes.*/
    void                           *outstandingCBData; /**< Data passed back to
                                                         the outstanding
                                                         callback */
